...
export type UNMERGE = "DD" | "AU" | "UA" | "UD" | "DU" | "AA" | "UU";

// see https://git-scm.com/docs/git-status
export const UNMERGE_STATUS: UNMERGE[] = [
	"DD",
	"AU",
	"UA",
	"UD",
...
export const forceChangeBranch = async function(
	repoDirPath: string,
	branchName: string
) {
...
const checkIsPushSuccess = async function(repoDirPath: string) {
	const currentBranch = await getCurrentBranch(repoDirPath);
	const localRev = (await execCommand(
		`git rev-parse --verify ${currentBranch}`,
		repoDirPath
	)).trim();
	const remoteRev = (await execCommand(
...
export const changeBranchSafely = async function(repoDirPath: string, branch: string) {
    await resetHard(repoDirPath, "HEAD");
    await cleanCurrentBranch(repoDirPath);
    await forceChangeBranch(repoDirPath, branch);
    await pullCurrentBranch(repoDirPath);
};

export const clone = async function(remoteRepoPath: string, cwd: string, dirname: string = "") {
    return await execCommand(`git clone ${remoteRepoPath} ${dirname}`, cwd);
};
...
export const submitCommit = async function(
	repoDirPath: string,
	commitMessage: string,
	modifyFileChecker?: ModifyFileChecker | string | string[]
) {
	if (await isNeedCommit(repoDirPath)) {
		modifyFileChecker =
			typeof modifyFileChecker !== "function" &&
			typeof modifyFileChecker !== "undefined"
				? buildChecker(modifyFileChecker)
				: modifyFileChecker;
		...
		const modifiedFiles =
			typeof modifyFileChecker !== "undefined" ? modifyFileChecker(files) : [];
...
export const hasConflict = async function(repoDirPath: string) {
	...
	const conflict = UNMERGE_STATUS.some( mergeStatus => Array.isArray(status[mergeStatus]) && status[mergeStatus].length > 0);
	return conflict;
};
